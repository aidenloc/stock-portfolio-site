...
### 4.7a Micro-interactions and mobile
- **Count-up on the headline value** (`CountUpMoney`). Three guards keep an animation on the first number a reader sees from being a liability: it honours `prefers-reduced-motion` (verified with Playwright's `reducedMotion: 'reduce'` — exactly one frame renders, the final value, with no flash of `$0.00` because the initial state is seeded from the media query rather than always starting at zero); the `<h1>` carries the true figure as an `aria-label` so assistive tech never announces intermediate numbers; and a ref guard means it runs **once** — `totalValue` is all-time, so switching periods must not re-trigger it. All three are covered by the verification script.
- **Hover/contrast on navigation.** Nav links were `text-gray-500`, the same value measured at 4.19:1 in the Section 3 contrast pass — under AA. Moved to `gray-400`, with `aria-current="page"` on the active link so the current page isn't signalled by weight/colour alone. Footer admin link and certification cards got matching hover transitions.
- **Header spacing at 375px.** A bare `justify-between` put the brand and the first nav link almost touching mid-line on a narrow phone; now `flex-wrap` with `gap-x-4` and a `gap-4 sm:gap-6` nav.
- **375px audit** across `/`, `/experience` and `/projects`: `document.body.scrollWidth === window.innerWidth` on all three (no horizontal overflow), with a DOM walk for any element extending past the viewport that isn't inside a deliberate `overflow-x-auto` container (the holdings table is the only one, by design).
...
- **Chart interpolation is `type="linear"`, never `"monotone"`.** A monotone (or natural) spline fits a cubic curve *through* the closes, so between any two points it can bow above or below both of them — drawing a price the portfolio never traded at. On a chart whose whole purpose is representing real performance to a finance reader, that's a correctness problem, not a style preference. Straight segments plot only values that actually occurred, matching Google Finance and other real price charts. Applied to the portfolio `Area` and the SPY `Line` together so the two stay visually consistent. The gradient fill is unchanged; its top edge simply follows the same angular path now, since an `Area`'s fill boundary and its stroke share one curve. Verified by reading the rendered SVG `d` attribute rather than by eye: all three paths (`area-area` fill, `area-curve` stroke, `line-curve` benchmark) contain zero `C` (cubic bézier) commands and only `M`/`L`. `app/StockChart.tsx` (the per-ticker modal) was switched to `linear` in the same pass, for the same reason.
