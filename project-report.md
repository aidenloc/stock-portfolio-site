...
- **Design decisions made for this feature** (resolving the open questions from the old Section 9.1): shown as a prominent section on the public homepage (not a separate route); overlays an S&P 500 (SPY) benchmark line from the start; positions are **not** back-datable — admin enters a ticker + share count, and the server stamps `entry_price` (current Finnhub quote at add-time) and `entry_date` (today) automatically.
...
- **`lib/yahooHistory.ts`** — shared Yahoo Finance history helper, extracted from `app/api/history/route.ts` so both that route and the new performance route use the same fetch/parse logic. Adds a `5Y` range (weekly interval) and a `periodSince(date)` helper that picks the smallest Yahoo range bucket covering a given start date (Yahoo only accepts fixed range buckets, not arbitrary spans).
- **`app/api/paper-portfolio/route.ts`** — GET (public, lists holdings), POST (admin-only: takes `{ ticker, shares }`, fetches the live Finnhub quote server-side to set `entry_price`, sets `entry_date` to today, inserts via `supabaseAdmin`), DELETE (admin-only, by id).
- **`app/api/paper-portfolio/performance/route.ts`** — GET (public). Computes an ARKK-fund-page-style time series:
  - Finds the earliest `entry_date` across all holdings, picks a Yahoo range via `periodSince`, and fetches history for every held ticker plus `SPY` in parallel.
  - Builds a per-ticker date→price map and forward-fills gaps (a holding's price on a day it didn't trade uses the most recent known close).
  - For each date, sums `shares × price` (only across holdings whose `entry_date` has passed) for portfolio value, and `shares × entry_price` for cost basis, then derives `portfolioReturnPct`. SPY is normalized to 0% at the first date in the series so both lines are directly comparable.
  - Also returns a holdings breakdown (current price/value, $ and % gain per position) and portfolio-wide totals.
- **`app/admin/PaperPortfolioForm.tsx`** — admin UI: ticker + shares inputs, list of current holdings with entry price/date and a remove button. Composed into `app/admin/page.tsx` alongside the existing forms.
- **`app/PaperPortfolio.tsx`** — public homepage component: headline total value + all-time return %, a Recharts line chart (portfolio % return vs. SPY % return, dashed benchmark line), and a holdings table (ticker, shares, value, gain/loss $ and %). Renders nothing if there are no holdings yet or the table isn't reachable, so it fails safe on the public homepage. Composed into `app/page.tsx` above the existing live price list.
...
**Built — see Section 4.9.** Remaining follow-ups, if wanted later: allow back-dated entries (would need admin-entered `entry_date`/`entry_price` fields instead of always stamping "today"); a dedicated `/paper-portfolio` route if the homepage section grows too large; additional benchmarks beyond SPY.
